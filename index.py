from tkinter import *
ventana=Tk()
ventana.title("Calculadora de jesus")
ventana.iconbitmap("icon.ico")
indice=0
#Entrada
entri=Entry(ventana,font=("Calibri 20"))
entri.grid(row=0,column=0,columnspan=4,padx=5,pady=5)
#Funciones
def click_boton(valor):
    global indice
    entri.insert(indice,valor)
    indice+=1
def delete():
    entri.delete(0,END)
    indice=0
...
boton_delete=Button(ventana,text="AC",width=5,height=2,command=lambda:delete())
boton_parent1=Button(ventana,text="(",width=5,height=2,command=lambda:click_boton("("))
boton_parent2=Button(ventana,text=")",width=5,height=2,command=lambda:click_boton(")"))
boton_point=Button(ventana,text=".",width=5,height=2,command=lambda:click_boton("."))
...
boton_igual=Button(ventana,text="=",width=5,height=2,command=lambda:operations())
